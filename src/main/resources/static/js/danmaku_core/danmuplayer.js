...
(function($) {
	...
	var DanmuPlayer = function(element, options) {
		...
		danmu_video = videojs("#danmu_video", {
			...
			function query() {
				//$("#danmu_text").remove("disabled");
				$("#danmu_text").removeAttr("disable");
				$("#danmu_text").attr("disabled", false);
				$.get(options.url_to_get_danmu, function(data, status) {
					var danmuList = JSON.parse(data);
					var danmu_from_sql = eval('(' +data+ ')');
					for (var i = 0; i < danmuList.length; i++) {
						//var danmu_ls = eval('(' + danmu_from_sql[i] + ')');
						var text_obj = '{ "text":"' + danmuList[i].text + '","color":"' + danmuList[i].color + '","size":"' + danmuList[i].size + '","position":"' + danmuList[i].position + '","time":' + danmuList[i].time + '}';
						var new_obj = eval('('+text_obj+')');
						$('#danmu71452').danmu("add_danmu", new_obj);
					}
				});
			};
			...
			$("body").append("<div id='tip2' class='tipb' hidden='true'><form  id='danmu_position'>弹幕位置：<input type='radio' checked='checked'  name='danmu_position' value='0' />滚动&nbsp;&nbsp;<input type='radio' name='danmu_position' value='1' />顶端&nbsp;&nbsp;<input type='radio' name='danmu_position' value='2' />底端&nbsp;&nbsp;</form><form  id='danmu_size' >弹幕大小：<input   type='radio' checked='checked'  name='danmu_size' value='1' />大&nbsp;&nbsp;<input   type='radio' n name='danmu_size' value='0' />小&nbsp;&nbsp;</form>弹幕颜色：<br><div id='danmu_color' /></div></div><div id='tip22' class='tipb' hidden='true'>透明度：<input type='range' name='op' id='op' onchange='op()' value=100 ><br>显示弹幕:<input type='checkbox' checked='checked' id='ishide' value='is' onchange='changehide()'> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;循环播放:<input type='checkbox' id='isloop' value='is' onchange='changeloop()'> </div> ");

			//$(".vjs-control-bar").append('<span class="shezhi " id="danmu_send_opt">弹幕设置</span>');
			...
			$(".vjs-control-bar").append('<span  class="shezhi  vjs-menu-button" id="danmu_shi_opt"  > 视频设置 </span>');
...
jQuery(document).ready(function() {
	jQuery("body").keydown(function(event) {
		if (event.which == 13) {
			console.log("enter")
			if ($("#LoginStatus").attr("value")=="notLogin")
			{
				layer.msg('请先登录!');
				return false;
			}
...
function send_danmu() {
	...
	var now = new Date();
	var StrTime = now.getMonth()+"-"+now.getDate()+" "+now.getHours()+":"+now.getMinutes();
	var St="&nbsp;&nbsp;";
	if ((now.getMonth()+"").length==1)
		St =(St)+"0";
	St=St+(now.getMonth()+1);
	St = St+"-";
	...
	if ((now.getMinutes()+"").length==1)
		St =St+"0";
	St=St+now.getMinutes();
	var Vtime = "";
	var MM = parseInt(time/600);
	var SS = parseInt(time%600);
	MM = MM.toFixed(0);
	SS = SS.toFixed(0);
	if(MM<100)
		Vtime = Vtime+"0";
	...
	if(SS<100)
		Vtime = Vtime + "0";
	Vtime = Vtime +parseInt(SS/10);

	var StringDanmaku = "<li class='danmaku-info-row bpui-selected' style='padding-top: 0px;'>" +
		"<span class='danmaku-info-time'>"+Vtime.replace('undefined')
		+"</span>" +
		"<span class='danmaku-info-danmaku' title='"+ text+"'>"+ text+"</span>" +
		"<span class='danmaku-info-date' title='"+St+"'>"+St+"</span>" +
		"</li>\n"
